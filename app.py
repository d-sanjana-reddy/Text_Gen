...
st.set_page_config(page_title="Text Generation + Classification", layout="wide")
st.title("🧠 Text Generation + Classification (Transformer + ML)")

# --- Load Dataset ---
@st.cache_data(show_spinner=False)
def load_data():
    url = "https://drive.google.com/uc?id=1AAz4llKgAAgouEhAZCDN2eqVhVrEmU_D"
    df = pd.read_csv(url)
...
# Load data and models with spinner
...
# Prompt input
st.subheader("✍️ Enter a text prompt for GPT-2:")
...
if st.button("🔄 Generate Text + Train Classifiers"):
    if user_input.strip() == "":
        ...
        with st.spinner("⚙️ Generating BERT embeddings..."):
